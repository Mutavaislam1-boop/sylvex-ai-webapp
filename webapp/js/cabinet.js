...
function renderGeneratedTelegramButton(url, kind) {
  ...
  const HOME_AI_MODELS = [
    ['seedream','Seedream 5.0 Lite'],['seedream','Seedream 4.5'],['ideogram','Ideogram 4.0'],['recraft','Recraft V4.1 Pro'],['openai','GPT Image 2'],['flux','FLUX.2 Turbo'],['qwen','Qwen Image 2 Pro'],['gemini','Nano Banana 2'],['google','Imagen 4 Ultra'],['grok','Grok Pro'],
  ...
  function renderHomeAiModels(){const host=document.getElementById('homeAiModelStrip');if(!host)return;const chips=HOME_AI_MODELS.map(([family,label])=>'<span class="ai-chip ai-'+family+'"><i>'+homeModelLogo(family)+'</i><b>'+S.escapeHtml(label)+'</b></span>').join('');host.innerHTML=chips+chips}
  ...
  function renderCommunityFeed() {
    ...
    host.innerHTML=communityItems.map(item=>{const type=normalizeGalleryType(item.type),media=String(item.media_url||''),urls=(item.media_urls||[media]).filter(Boolean).slice(0,4),avatar=item.author_avatar?'<img src="'+S.escapeHtml(item.author_avatar)+'" alt="">':S.escapeHtml(String(item.author_name||'S').slice(0,2).toUpperCase()),ago=item.created_at?new Date(item.created_at).toLocaleDateString():'недавно',rank=communityRank(item.author_likes||item.likes);let content='';if(type==='image'&&urls.length)content='<div class="community-photo-grid count-'+urls.length+'">'+urls.map(url=>'<img src="'+S.escapeHtml(url)+'" alt="" loading="lazy">').join('')+'</div>';else if(type==='video'&&media)content='<video src="'+S.escapeHtml(media)+'" controls playsinline preload="metadata"></video>';else if((type==='music'||type==='voice')&&media)content='<div class="community-audio">'+profileGalleryIcon(type)+'<audio controls src="'+S.escapeHtml(media)+'"></audio></div>';else content='<div class="community-text">'+S.escapeHtml(item.body||'Публикация')+'</div>';const id=Number(item.id||0);return '<article class="community-post" data-community-search="'+S.escapeHtml((item.author_name+' '+item.body+' '+item.model).toLowerCase())+'"><header><button class="community-author-button" onclick="SYLVEX.openCommunityUserCard('+Number(item.author_id||0)+')"><span class="community-avatar" data-rank="'+rank.level+'">'+avatar+'</span><span><b>'+S.escapeHtml(item.author_name||'SYLVEX User')+' <em>'+rank.icon+'</em></b><small>'+S.escapeHtml(ago)+' · '+rank.name+'</small></span></button>'+(item.demo?'':'<span class="community-author-actions"><button onclick="SYLVEX.followCommunityAuthor('+Number(item.author_id||0)+')">Подписаться</button><button onclick="SYLVEX.messageCommunityAuthor('+Number(item.author_id||0)+')">Написать</button></span>')+'</header>'+(item.body?'<p class="community-caption">'+S.escapeHtml(item.body)+'</p>':'')+'<div class="community-media">'+content+'</div><footer><button class="'+(item.liked?'liked':'')+'" type="button" '+(item.demo?'onclick="SYLVEX.communityComingSoon(\'Демо публикация\')"':'onclick="SYLVEX.toggleCommunityLike('+id+')"')+'>'+communitySvg('heart')+'<span>'+Number(item.likes||0)+'</span></button><button type="button" '+(item.demo?'onclick="SYLVEX.communityComingSoon(\'Демо публикация\')"':'onclick="SYLVEX.openCommunityComments('+id+')"')+'>'+communitySvg('comment')+'<span>'+Number(item.comments||0)+'</span></button><button type="button" onclick="SYLVEX.shareCommunityPost('+id+')">'+communitySvg('send')+'</button><button type="button" onclick="SYLVEX.downloadCommunityPost(\''+S.escapeHtml(media)+'\')">'+profileGalleryIcon('download')+'</button></footer></article>'}).join('');
  }
  ...
  async function openCommunityPublisher(){await loadProfileGallery(false);let modal=document.getElementById('communityPublishModal');if(!modal){modal=document.createElement('div');modal.id='communityPublishModal';modal.className='community-modal';document.body.appendChild(modal)}const choices=profileGalleryItems.map(item=>'<label class="community-publish-choice"><input type="checkbox" value="'+S.escapeHtml(String(item.id))+'" data-kind="'+normalizeGalleryType(item.type)+'" onchange="SYLVEX.limitCommunitySelection(this)"><span>'+profileGalleryIcon(normalizeGalleryType(item.type))+'</span><b>'+S.escapeHtml(String(item.prompt||item.text||'Генерация').slice(0,70))+'</b></label>').join('');modal.innerHTML='<section><header><div><small>Новая публикация</small><h3>Выберите до 4 фотографий</h3></div><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><textarea id="communityCaption" maxlength="360" rows="3" placeholder="Подпись — не более трёх строк, без сторонних ссылок"></textarea><div class="community-publish-list">'+(choices||'<p>Сначала создайте работу в Pro Studio.</p>')+'</div><button class="community-publish-submit" onclick="SYLVEX.publishCommunitySelection()">Опубликовать</button></section>';modal.classList.add('show')}
  function limitCommunitySelection(input){const checked=Array.from(document.querySelectorAll('#communityPublishModal input:checked'));if(checked.length>4){input.checked=false;toast('Можно выбрать не более четырёх фотографий');return}if(checked.length>1&&checked.some(el=>el.dataset.kind!=='image')){input.checked=false;toast('Несколько файлов можно объединять только для фотографий')}}
...
async function waitGeneration(jobId, options) {
  ...
  Object.assign(S, {
    ...
    openCommunityUserCard, followCommunityAuthor, messageCommunityAuthor, shareCommunityPost, downloadCommunityPost, limitCommunitySelection, publishCommunitySelection,
