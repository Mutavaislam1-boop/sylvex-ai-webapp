...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function renderCommunitySideContent(){const stories=document.getElementById('communityStories'),popular=document.getElementById('communityPopular');const authors=[];communityItems.forEach(item=>{if(!authors.some(a=>String(a.author_id)===String(item.author_id)))authors.push(item)});const own=S.user||{};const all=[{author_name:'Вы',author_avatar:own.custom_avatar_url||own.photo_url||''}].concat(authors).slice(0,7);const avatarMarkup=a=>a.author_avatar?'<img src="'+S.escapeHtml(a.author_avatar)+'" alt="">':S.escapeHtml(String(a.author_name||'SY').slice(0,2).toUpperCase());if(stories)stories.innerHTML=all.map((a,i)=>'<button type="button" onclick="SYLVEX.communityComingSoon(\'Истории\')"><i class="'+(i===0?'own':'')+'"><span>'+avatarMarkup(a)+'</span>'+(i===0?'<b>+</b>':'')+'</i><small>'+S.escapeHtml(a.author_name||'Автор')+'</small></button>').join('');if(popular)popular.innerHTML=authors.slice(0,4).map(a=>'<button type="button" onclick="SYLVEX.communityComingSoon(\'Профиль автора\')"><span class="community-avatar">'+avatarMarkup(a)+'</span><span><b>'+S.escapeHtml(a.author_name||'SYLVEX User')+'</b><small>'+S.escapeHtml(a.author_username?'@'+a.author_username:'Автор SYLVEX')+'</small></span><i>Подписаться</i></button>').join('')||'<p>Авторы появятся после первых публикаций.</p>';const composer=document.getElementById('communityComposerAvatar');if(composer)composer.innerHTML=avatarMarkup({author_name:own.display_name||own.first_name||'SY',author_avatar:own.custom_avatar_url||own.photo_url||''})}
  function renderCommunityFeed() {
    ...
    host.innerHTML=communityItems.map(item=>{const type=normalizeGalleryType(item.type),media=String(item.media_url||''),preview=String(item.preview_url||media||''),avatar=item.author_avatar?'<img src="'+S.escapeHtml(item.author_avatar)+'" alt="">':S.escapeHtml(String(item.author_name||'S').slice(0,2).toUpperCase()),ago=item.created_at?new Date(item.created_at).toLocaleDateString():'недавно';let content='';if(type==='image'&&preview)content='<img src="'+S.escapeHtml(preview)+'" alt="" loading="lazy">';else if(type==='video'&&media)content='<video src="'+S.escapeHtml(media)+'" controls playsinline preload="metadata"></video>';else if((type==='music'||type==='voice')&&media)content='<div class="community-audio">'+profileGalleryIcon(type)+'<audio controls src="'+S.escapeHtml(media)+'"></audio></div>';else content='<div class="community-text">'+S.escapeHtml(item.body||'Публикация')+'</div>';return '<article class="community-post" data-community-search="'+S.escapeHtml((item.author_name+' '+item.body+' '+item.model).toLowerCase())+'"><header><span class="community-avatar">'+avatar+'</span><div><b>'+S.escapeHtml(item.author_name||'SYLVEX User')+' <em>◆</em></b><small>'+S.escapeHtml(ago)+'</small></div><button class="community-more">•••</button></header>'+(item.body?'<p>'+S.escapeHtml(item.body)+'</p>':'')+'<div class="community-media">'+content+'</div><footer><button class="'+(item.liked?'liked':'')+'" type="button" onclick="SYLVEX.toggleCommunityLike('+Number(item.id)+')">'+communitySvg('heart')+'<span>'+Number(item.likes||0)+'</span></button><button type="button" onclick="SYLVEX.openCommunityComments('+Number(item.id)+')">'+communitySvg('comment')+'<span>'+Number(item.comments||0)+'</span></button><button type="button" onclick="SYLVEX.communityComingSoon(\'Поделиться\')">'+communitySvg('send')+'</button><button class="community-save" type="button" onclick="SYLVEX.communityComingSoon(\'Сохранено\')"><svg viewBox="0 0 24 24"><path d="M6 3h12v18l-6-4-6 4Z"/></svg></button></footer></article>'}).join('');
  }
  ...
  function toggleCommunityMenu(){document.querySelector('.community-app')?.classList.toggle('menu-open')}
  function communityComingSoon(name){toast(name+' — скоро')}
  async function loadCommunityFeed(force){if(communityItems.length&&!force)return renderCommunityFeed();const host=document.getElementById('communityFeed');if(host)host.innerHTML='<div class="community-state">Загрузка публикаций…</div>';try{const r=await fetch('/api/public/community/feed?telegram_id='+encodeURIComponent(getTelegramId()||0),{cache:'no-store'});const j=await r.json();communityItems=Array.isArray(j.items)?j.items:[];renderCommunityFeed()}catch(_){if(host)host.innerHTML='<div class="community-state">Не удалось загрузить сообщество.</div>'}}
  async function toggleCommunityLike(id){const item=communityItems.find(x=>Number(x.id)===Number(id));if(item){item.liked=!item.liked;item.likes=Math.max(0,Number(item.likes||0)+(item.liked?1:-1));renderCommunityFeed()}try{const r=await fetch('/api/public/community/posts/'+id+'/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),initData:S.tg&&S.tg.initData||''})});const j=await r.json();if(item)item.liked=!!j.liked}catch(_){loadCommunityFeed(true)}}
  async function openCommunityPublisher(){await loadProfileGallery(false);let modal=document.getElementById('communityPublishModal');if(!modal){modal=document.createElement('div');modal.id='communityPublishModal';modal.className='community-modal';document.body.appendChild(modal)}const choices=profileGalleryItems.map(item=>'<button type="button" onclick="SYLVEX.publishCommunityItem(\''+S.escapeHtml(String(item.id))+'\')"><span>'+profileGalleryIcon(normalizeGalleryType(item.type))+'</span><b>'+S.escapeHtml(String(item.prompt||item.text||'Генерация').slice(0,70))+'</b></button>').join('');modal.innerHTML='<section><header><div><small>Новая публикация</small><h3>Выберите работу</h3></div><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><div class="community-publish-list">'+(choices||'<p>Сначала создайте работу в Pro Studio.</p>')+'</div></section>';modal.classList.add('show')}
  async function publishCommunityItem(id){try{const r=await fetch('/api/public/community/posts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),message_id:Number(id),initData:S.tg&&S.tg.initData||''})});const j=await r.json();if(!r.ok||j.error)throw new Error(j.error||'publish_failed');document.getElementById('communityPublishModal').classList.remove('show');toast('Работа опубликована');loadCommunityFeed(true)}catch(_){toast('Не удалось опубликовать работу')}}
  async function openCommunityComments(id){let modal=document.getElementById('communityCommentsModal');if(!modal){modal=document.createElement('div');modal.id='communityCommentsModal';modal.className='community-modal';document.body.appendChild(modal)}modal.innerHTML='<section><header><h3>Комментарии</h3><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><div class="community-comments">Загрузка…</div><form onsubmit="SYLVEX.sendCommunityComment(event,'+id+')"><input name="comment" maxlength="500" placeholder="Написать комментарий…" required><button type="submit">'+communitySvg('send')+'</button></form></section>';modal.classList.add('show');try{const r=await fetch('/api/public/community/posts/'+id+'/comments');const j=await r.json();modal.querySelector('.community-comments').innerHTML=(j.items||[]).map(c=>'<div><span class="community-avatar">'+(c.author_avatar?'<img src="'+S.escapeHtml(c.author_avatar)+'" alt="">':S.escapeHtml(String(c.author_name||'S').slice(0,2)))+'</span><p><b>'+S.escapeHtml(c.author_name||'User')+'</b>'+S.escapeHtml(c.body)+'</p></div>').join('')||'<p>Комментариев пока нет.</p>'}catch(_){modal.querySelector('.community-comments').textContent='Не удалось загрузить комментарии.'}}
  async function sendCommunityComment(event,id){event.preventDefault();const input=event.currentTarget.elements.comment,body=input.value.trim();if(!body)return;input.value='';await fetch('/api/public/community/posts/'+id+'/comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),body,initData:S.tg&&S.tg.initData||''})});const item=communityItems.find(x=>Number(x.id)===Number(id));if(item)item.comments=Number(item.comments||0)+1;renderCommunityFeed();openCommunityComments(id)}
...
async function waitGeneration(jobId, options) {
  ...
  Object.assign(S, {
    ...
    loadCommunityFeed, toggleCommunityLike, openCommunityPublisher, publishCommunityItem, openCommunityComments, sendCommunityComment, searchCommunity, toggleCommunityMenu, communityComingSoon,
