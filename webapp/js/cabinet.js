...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function renderBrandGeneratorHeader(config,key){const studio=document.querySelector('[data-view="tools"] .studio');if(!studio)return;let head=studio.querySelector('.brand-generator-head');if(!head){head=document.createElement('div');head.className='brand-generator-head';studio.prepend(head)}head.innerHTML='<div class="brand-generator-name">'+(config.logo?'<img src="'+S.escapeHtml(config.logo)+'" alt="">':'<span></span>')+'<b>'+S.escapeHtml(config.label)+'</b></div><div class="brand-generator-actions"><span>SYLVEX</span><button type="button" onclick="SYLVEX.closeBrandGenerator(event)" aria-label="Закрыть">×</button></div>';head.dataset.brand=key}
  function openBrandGenerator(event,key){if(event){event.preventDefault();event.stopPropagation()}const config=BRAND_GENERATORS[key];if(!config)return;switchView('tools');document.body.classList.add('brand-generator-active');document.body.dataset.brandGenerator=key;if(config.mode==='image')imageState.modelId=config.model;else if(config.mode==='video')videoState.modelId=config.model;else if(config.mode==='music')musicState.modelId=config.model;else if(config.mode==='voice')voiceState.modelId=config.model;else if(config.mode==='text')textState.modelId=config.model;updateComposerMode(config.mode);renderBrandGeneratorHeader(config,key);renderModelPop();if(config.mode==='image')renderImageControls();else if(config.mode==='video')renderVideoControls();else if(config.mode==='music')renderMusicControls();else if(config.mode==='voice')renderVoiceControls();else if(config.mode==='text')renderTextControls();updateSendButton()}
  function closeBrandGenerator(event){if(event){event.preventDefault();event.stopPropagation()}document.body.classList.remove('brand-generator-active');delete document.body.dataset.brandGenerator;document.querySelector('.brand-generator-head')?.remove();switchView('home')}
