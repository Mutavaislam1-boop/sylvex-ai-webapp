...
function renderGeneratedTelegramButton(url, kind) {
  ...
  async function openCommunityPublisher(){await loadProfileGallery(false);let modal=document.getElementById('communityPublishModal');if(!modal){modal=document.createElement('div');modal.id='communityPublishModal';modal.className='community-modal';document.body.appendChild(modal)}const allowed=new Set(['image','video']),items=profileGalleryItems.filter(item=>allowed.has(normalizeGalleryType(item.type))),choices=items.map(item=>{const kind=normalizeGalleryType(item.type);return '<label class="community-publish-choice" data-kind="'+kind+'" hidden><input type="checkbox" value="'+S.escapeHtml(String(item.id))+'" data-kind="'+kind+'" onchange="SYLVEX.limitCommunitySelection(this)"><span>'+profileGalleryIcon(kind)+'</span><b>'+S.escapeHtml(String(item.prompt||item.text||'Генерация').slice(0,70))+'</b></label>'}).join(''),typeButton=(kind,label)=>'<button type="button" data-kind="'+kind+'" onclick="SYLVEX.filterCommunityPublisher(\''+kind+'\')"><span>'+profileGalleryIcon(kind)+'</span><b>'+label+'</b></button>';modal.innerHTML='<section><header><div><small>Новая публикация</small><h3>Выберите медиа</h3></div><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><div class="community-publish-types">'+typeButton('image','Фото')+typeButton('video','Видео')+'</div><div class="community-publish-list">'+(choices||'<p>Сначала создайте фото или видео в Pro Studio.</p>')+'</div><div class="community-caption-field" hidden><textarea id="communityCaption" maxlength="360" rows="3" placeholder="Добавьте подпись…"></textarea><small>Необязательно</small></div><button class="community-publish-submit" onclick="SYLVEX.publishCommunitySelection()">Опубликовать</button></section>';modal.classList.add('show')}
