...
def request_byteplus_seedream_image(model: str, prompt: str) -> list:
    response = requests.post(
    ...
    try:
        return normalize_image_response(response.json())
    except Exception:
        return []
...
def generateBytePlusSeedreamImage(payload: dict) -> dict:
    ...
    try:
        ...
        for _ in range(count):
            for url in request_byteplus_seedream_image(model, prompt):
