...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function ensureCommunityFixedNav(){const nav=document.querySelector('.community-mobile-nav');if(!nav||nav.parentNode===document.body)return;if(!communityNavMarker){communityNavMarker=document.createComment('community-mobile-nav-home');nav.parentNode.insertBefore(communityNavMarker,nav)}document.body.appendChild(nav)}
  ...
  async function openCommunityHub(tab='messages',peerId=null){toggleCommunityMenu(false);const modal=communityHubModal();modal.classList.add('show');modal.innerHTML='<section><header><b>SYLVEX Community</b><button type="button" onclick="SYLVEX.closeCommunityHub()">'+communitySvg('close')+'</button></header><main><div class="community-state">Загрузка…</div></main></section>';try{if(tab==='notifications'){const r=await fetch('/api/public/community/notifications?telegram_id='+encodeURIComponent(getTelegramId()||0)),j=await r.json();renderCommunityNotifications(modal,j.items||[]);fetch('/api/public/community/notifications/read',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId()})}).then(loadCommunityNotificationBadge);return}const hr=await fetch('/api/public/community/hub?telegram_id='+encodeURIComponent(getTelegramId()||0)),hub=await hr.json();if(tab==='friends'){renderCommunityFriends(modal,hub);return}if(tab==='chat'){await renderCommunityChat(modal,Number(peerId||0),hub);return}renderCommunityConversations(modal,hub)}catch(_){modal.querySelector('main').innerHTML='<div class="community-state">Не удалось загрузить раздел.</div>'}}
  function closeCommunityHub(){document.getElementById('communityHubModal')?.classList.remove('show')}
  ...
  async function renderCommunityChat(modal,peerId,hub){const peer=peerId===0?{name:'Общий чат'}:[...(hub.friends||[]),...(hub.conversations||[])].find(item=>Number(item.id)===peerId)||communityItems.find(item=>Number(item.author_id)===peerId)||{name:'Личные сообщения'};modal.querySelector('header b').textContent=peer.name||peer.author_name||'Чат';const r=await fetch('/api/public/community/messages/'+peerId+'?telegram_id='+encodeURIComponent(getTelegramId()||0)),j=await r.json();modal.dataset.peerId=peerId;modal.querySelector('main').innerHTML='<div class="community-chat-messages">'+((j.items||[]).map(item=>'<article class="'+(item.is_own?'own':'')+'"><b>'+S.escapeHtml(item.author_name||'SYLVEX User')+'</b><p>'+S.escapeHtml(item.body)+'</p><time>'+S.escapeHtml(communityCommentTime(item.created_at))+'</time></article>').join('')||'<div class="community-state">Начните общение первым.</div>')+'</div><form class="community-chat-form" onsubmit="SYLVEX.sendCommunityMessage(event,'+peerId+')"><input name="message" maxlength="2000" autocomplete="off" placeholder="Сообщение…" required><button type="submit">'+communitySvg('send')+'</button></form>';const list=modal.querySelector('.community-chat-messages');list.scrollTop=list.scrollHeight}
  async function sendCommunityMessage(event,recipientId){event.preventDefault();const form=event.currentTarget,input=form.elements.message,body=input.value.trim();if(!body)return;input.disabled=true;try{const r=await fetch('/api/public/community/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),recipient_id:recipientId,body,initData:S.tg&&S.tg.initData||''})});if(!r.ok)throw new Error();input.value='';await openCommunityHub('chat',recipientId)}catch(_){toast('Не удалось отправить сообщение')}finally{input.disabled=false}}
  ...
  function renderCommunityNotifications(modal,items){modal.querySelector('header b').textContent='Уведомления';modal.querySelector('main').innerHTML='<nav class="community-hub-tabs"><button onclick="SYLVEX.openCommunityHub(\'messages\')">Сообщения</button><button onclick="SYLVEX.openCommunityHub(\'friends\')">Друзья</button><button class="active">Уведомления</button></nav><div class="community-notifications">'+(items.map(item=>'<article class="'+(item.read?'':'unread')+'"><span class="community-avatar">'+(item.actor_avatar?'<img src="'+S.escapeHtml(item.actor_avatar)+'" alt="">':S.escapeHtml(String(item.actor_name||'SY').slice(0,2)))+'</span><div><b>'+S.escapeHtml(item.actor_name||'Пользователь')+'</b><p>'+S.escapeHtml(item.body||({like:'поставил(а) лайк',comment:'оставил(а) комментарий',friend_request:'отправил(а) запрос в друзья',message:'отправил(а) сообщение'}[item.kind]||'Новое событие'))+'</p><time>'+S.escapeHtml(communityCommentTime(item.created_at))+'</time></div></article>').join('')||'<div class="community-state">Новых уведомлений нет.</div>')+'</div>'}
...
async function waitGeneration(jobId, options) {
  ...
  Object.assign(S, {
    ...
    openCommunityHub, closeCommunityHub, sendCommunityMessage, sendCommunityFriendRequest, communityFriendAction, loadCommunityNotificationBadge,
