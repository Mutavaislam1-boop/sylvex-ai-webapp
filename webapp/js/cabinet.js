...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function enhanceCommunityMedia(){document.querySelectorAll('.community-post').forEach(post=>{const item=communityItems.find(value=>Number(value.id)===Number(post.dataset.postId)),media=post.querySelector('.community-media'),video=media?.querySelector('video'),audioBox=media?.querySelector('.community-audio');if(video&&!media.querySelector('.community-video-ui')){video.controls=false;video.muted=true;video.removeAttribute('controls');const ui=document.createElement('div');ui.className='community-video-ui';ui.innerHTML='<button type="button" data-community-sound aria-label="Включить звук">◌</button><time>00:00</time>';media.appendChild(ui);video.addEventListener('timeupdate',()=>{const timer=ui.querySelector('time');if(timer)timer.textContent=formatCommunityVideoTime(video.duration-video.currentTime)});ui.querySelector('button').addEventListener('click',event=>{event.stopPropagation();video.muted=!video.muted;event.currentTarget.textContent=video.muted?'◌':'◉';event.currentTarget.setAttribute('aria-label',video.muted?'Включить звук':'Выключить звук')})}if(audioBox&&!audioBox.querySelector('.community-music-cover')){const cover=document.createElement('img');cover.className='community-music-cover';cover.src=String(item?.preview_url||'assets/knowledge-center/music.png');cover.alt='Обложка трека';cover.onerror=()=>{cover.onerror=null;cover.src='assets/knowledge-center/music.png'};audioBox.prepend(cover)}})}
