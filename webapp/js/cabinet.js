...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function renderCommunityFeed() {
    ...
    if(!communityItems.length){host.innerHTML='<div class="community-state">Пока нет публикаций. Станьте первым автором сообщества.</div>';return;}
    host.innerHTML=communityItems.map(item=>{const type=normalizeGalleryType(item.type),media=String(item.media_url||''),preview=String(item.preview_url||media||''),avatar=item.author_avatar?'<img src="'+S.escapeHtml(item.author_avatar)+'" alt="">':S.escapeHtml(String(item.author_name||'S').slice(0,2).toUpperCase());let content='';if(type==='image'&&preview)content='<img src="'+S.escapeHtml(preview)+'" alt="" loading="lazy">';else if(type==='video'&&media)content='<video src="'+S.escapeHtml(media)+'" controls playsinline preload="metadata"></video>';else if((type==='music'||type==='voice')&&media)content='<div class="community-audio">'+profileGalleryIcon(type)+'<audio controls src="'+S.escapeHtml(media)+'"></audio></div>';else content='<div class="community-text">'+S.escapeHtml(item.body||'Публикация')+'</div>';return '<article class="community-post"><header><span class="community-avatar">'+avatar+'</span><div><b>'+S.escapeHtml(item.author_name||'SYLVEX User')+'</b><small>'+S.escapeHtml(item.author_username?'@'+item.author_username:(item.model||'SYLVEX'))+'</small></div></header><div class="community-media">'+content+'</div>'+(type!=='text'&&item.body?'<p>'+S.escapeHtml(item.body)+'</p>':'')+'<footer><button class="'+(item.liked?'liked':'')+'" type="button" onclick="SYLVEX.toggleCommunityLike('+Number(item.id)+')">'+communitySvg('heart')+'<span>'+Number(item.likes||0)+'</span></button><button type="button" onclick="SYLVEX.openCommunityComments('+Number(item.id)+')">'+communitySvg('comment')+'<span>'+Number(item.comments||0)+'</span></button></footer></article>'}).join('');
  }
...
async function waitGeneration(jobId, options) {
  ...
  Object.assign(S, {
    ...
    loadCommunityFeed, toggleCommunityLike, openCommunityPublisher, publishCommunityItem, openCommunityComments, sendCommunityComment,
