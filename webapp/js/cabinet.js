...
function renderGeneratedTelegramButton(url, kind) {
  ...
  function profileGalleryCacheKey(){return 'sylvex-profile-gallery-v1-'+(getTelegramId()||'guest')}
  function hydrateProfileGalleryCache(){if(profileGalleryItems.length)return true;try{const cached=JSON.parse(localStorage.getItem(profileGalleryCacheKey())||'null');if(cached&&Array.isArray(cached.items)){profileGalleryItems=cached.items;return true}}catch(_){}return false}
  function cacheProfileGallery(){try{localStorage.setItem(profileGalleryCacheKey(),JSON.stringify({savedAt:Date.now(),items:profileGalleryItems.slice(0,100)}))}catch(_){}}
  ...
  const BRAND_GENERATORS={openai:{label:'OpenAI',mode:'image',model:'gpt_image_2',logo:'assets/ai-brand-logos/openai.png'},gemini:{label:'Gemini',mode:'text',model:'gemini_3_1_pro',logo:'assets/ai-brand-logos/gemini.webp'},flux:{label:'FLUX',mode:'image',model:'flux_2',logo:''},kling:{label:'Kling AI',mode:'video',model:'kling_3_0',logo:'assets/ai-brand-logos/kling.png'},runway:{label:'Runway',mode:'video',model:'runway_gen4_5',logo:'assets/ai-brand-logos/runway.png'},suno:{label:'Suno',mode:'music',model:'suno_chirp_5_5',logo:'assets/ai-brand-logos/suno.png',headerVideo:'assets/brand-generators/suno-header.mp4'},elevenlabs:{label:'ElevenLabs',mode:'voice',model:'elevenlabs_eleven_v3',logo:'assets/ai-brand-logos/elevenlabs.jpeg'},seedream:{label:'Seedream 5.0 Pro',mode:'image',model:'seedream_5_0_pro',logo:'assets/ai-brand-logos/seedream.webp'},seedance:{label:'Seedance 2.0',mode:'video',model:'seedance_2_0',logo:'assets/ai-brand-logos/seedance.jpeg'},grok:{label:'Grok',mode:'text',model:'grok_4_1',logo:'assets/ai-brand-logos/grok.jpeg'},heygen:{label:'HeyGen Avatar',mode:'video',model:'heygen_avatar_v',logo:''}};
  const BRAND_MUSIC_FALLBACK=[
    {title:'Pop & Vocal',genre:'Pop',coverUrl:'assets/hero-ai/suno.jpg'},
    {title:'Ambient',genre:'Ambient',coverUrl:'assets/hero-ai/gemini.jpg'},
    {title:'Cinematic',genre:'Cinematic',coverUrl:'assets/hero-ai/seedream.jpg'},
    {title:'Hip-Hop',genre:'Hip-Hop',coverUrl:'assets/hero-ai/grok.jpg'},
  ...
  async function loadBrandMusicLibrary(){if(brandMusicLibraryLoaded)return;brandMusicLibraryLoaded=true;try{const response=await fetch('/api/public/prostudio/gallery?telegram_id='+encodeURIComponent(getTelegramId()||0)+'&limit=100',{cache:'no-store'}),data=await response.json(),tracks=(Array.isArray(data.items)?data.items:[]).filter(item=>{const type=String(item.type||'').toLowerCase(),source=String(item.provider||'')+' '+String(item.model||'');return !!item.media_url&&(type==='music'||(type==='audio'&&/suno|chirp/i.test(source)))}).map(item=>({title:String(item.title||item.prompt||'Трек Suno').slice(0,80),genre:item.genre||'Suno',audioUrl:item.media_url,coverUrl:item.preview_url||'assets/hero-ai/suno.jpg'}));if(!tracks.length||document.body.dataset.brandGenerator!=='suno')return;brandMusicTracks=tracks.concat(BRAND_MUSIC_FALLBACK);brandMusicIndex=0;renderBrandMusicShowcase()}catch(_){}}
  function formatBrandMusicTime(value){const seconds=Math.max(0,Math.floor(Number(value)||0));return String(Math.floor(seconds/60)).padStart(2,'0')+':'+String(seconds%60).padStart(2,'0')}
  function selectBrandMusic(index,play){const count=brandMusicTracks.length;if(!count)return;brandMusicIndex=(Number(index)+count)%count;updateBrandMusicSlider();const track=brandMusicTracks[brandMusicIndex],player=document.getElementById('brandMusicPlayer'),audio=player?.querySelector('audio');if(!player||!audio)return;player.querySelector('b').textContent=track.title||track.genre||'Suno';const cover=track.coverUrl||'assets/knowledge-center/music.png';player.querySelector('.brand-music-cover img').src=cover;player.style.setProperty('--brand-music-cover','url("'+String(cover).replace(/["\\]/g,'')+'")');if(audio.dataset.src!==String(track.audioUrl||'')){audio.pause();audio.src=track.audioUrl||'';audio.dataset.src=track.audioUrl||'';audio.load()}audio.ontimeupdate=()=>{const range=player.querySelector('input'),duration=audio.duration||0;range.value=duration?String(audio.currentTime/duration*100):'0';player.querySelector('[data-brand-time]').textContent=formatBrandMusicTime(audio.currentTime);player.querySelector('[data-brand-duration]').textContent=formatBrandMusicTime(duration)};audio.onplay=()=>{player.querySelector('.brand-music-play').textContent='Ⅱ'};audio.onpause=()=>{player.querySelector('.brand-music-play').textContent='▶'};if(play&&track.audioUrl)audio.play().catch(()=>{})}
  function moveBrandMusic(direction,play){selectBrandMusic(brandMusicIndex+(Number(direction)||0),!!play)}
  function toggleBrandMusic(){const audio=document.querySelector('#brandMusicPlayer audio');if(!audio||!audio.src)return;if(audio.paused)audio.play().catch(()=>{});else audio.pause()}
  function seekBrandMusic(percent){const audio=document.querySelector('#brandMusicPlayer audio');if(audio&&Number.isFinite(audio.duration))audio.currentTime=audio.duration*(Number(percent)||0)/100}
  ...
  let communityItems = [];
  let communityFeedLoadedAt = 0;
  let communityVideoObserver = null;
  let communityMenuSidebarMarker = null;
  let communityMenuBackdropMarker = null;
  let communityNavMarker = null;
  let communityScrollBound = false;
  let communityLastScrollTop = 0;
  let communityChatPollTimer = null;
  let communityFeedPromise = null;
  const communityCommentsCache = new Map();
  const communityCommentsPromises = new Map();
  function communityFeedCacheKey(){return 'sylvex-community-feed-v2-'+(getTelegramId()||'guest')}
  function readCommunityFeedCache(){try{const value=JSON.parse(localStorage.getItem(communityFeedCacheKey())||'null');return value&&Array.isArray(value.items)?value:null}catch(_){return null}}
  function writeCommunityFeedCache(items){try{localStorage.setItem(communityFeedCacheKey(),JSON.stringify({savedAt:Date.now(),items:(items||[]).filter(item=>!item.demo).slice(0,80)}))}catch(_){}}
  function communityFetch(url,options,timeoutMs){const controller=new AbortController(),timer=window.setTimeout(()=>controller.abort(),timeoutMs||9000);return fetch(url,Object.assign({},options||{},{signal:controller.signal})).finally(()=>window.clearTimeout(timer))}
  ...
  function renderCommunityFeed() {
    ...
    host.innerHTML=communityItems.map(item=>{const type=normalizeGalleryType(item.type),media=String(item.media_url||''),urls=(item.media_urls||[media]).filter(Boolean).slice(0,4),avatar=item.author_avatar?'<img src="'+S.escapeHtml(item.author_avatar)+'" alt="">':S.escapeHtml(String(item.author_name||'S').slice(0,2).toUpperCase()),ago=item.created_at?new Date(item.created_at).toLocaleDateString():'недавно',rank=communityRank(item.author_likes||item.likes);let content='';if(type==='image'&&urls.length)content='<div class="community-photo-grid count-'+urls.length+'">'+urls.map(url=>'<img src="'+S.escapeHtml(url)+'" alt="" loading="lazy" decoding="async">').join('')+'</div>';else if(type==='video'&&media)content='<video src="'+S.escapeHtml(media)+'" controls playsinline muted loop preload="metadata" data-community-video></video>';else if(type==='music'&&media)content='<div class="community-audio">'+profileGalleryIcon(type)+'<audio controls preload="metadata" src="'+S.escapeHtml(media)+'"></audio></div>';const id=Number(item.id||0),caption=String(item.body||'').trim();return '<article class="community-post" data-post-id="'+id+'" data-community-search="'+S.escapeHtml((String(item.author_name||'')+' '+caption+' '+String(item.model||'')).toLowerCase())+'"><header><button class="community-author-button" onclick="SYLVEX.openCommunityUserCard('+Number(item.author_id||0)+')"><span class="community-avatar" data-rank="'+rank.level+'">'+avatar+'</span><span><b>'+S.escapeHtml(item.author_name||'SYLVEX User')+' <em>'+rank.icon+'</em></b><small>'+S.escapeHtml(ago)+' · '+rank.name+'</small></span></button>'+(item.is_own?'<button class="community-delete-post" type="button" onclick="SYLVEX.deleteCommunityPost('+id+',this)" aria-label="Удалить публикацию">'+communitySvg('close')+'</button>':(item.demo?'':'<span class="community-author-actions"><button onclick="SYLVEX.sendCommunityFriendRequest('+Number(item.author_id||0)+')">В друзья</button><button onclick="SYLVEX.messageCommunityAuthor('+Number(item.author_id||0)+')">Написать</button></span>'))+'</header>'+(caption?'<p class="community-caption">'+S.escapeHtml(caption)+'</p>':'')+'<div class="community-media">'+content+'</div><footer><button data-community-like="'+id+'" class="'+(item.liked?'liked':'')+'" type="button" '+(item.demo?'onclick="SYLVEX.communityComingSoon(\'Демо публикация\')"':'onclick="SYLVEX.toggleCommunityLike('+id+',this)"')+'>'+communitySvg('heart')+'<span>'+Number(item.likes||0)+'</span></button><button type="button" '+(item.demo?'onclick="SYLVEX.communityComingSoon(\'Демо публикация\')"':'onclick="SYLVEX.openCommunityComments('+id+')"')+'>'+communitySvg('comment')+'<span>'+Number(item.comments||0)+'</span></button></footer></article>'}).join('');
    enhanceCommunityMedia();observeCommunityVideos();
  }
  ...
  function formatCommunityVideoTime(seconds){const value=Math.max(0,Math.floor(Number(seconds)||0));return String(Math.floor(value/60)).padStart(2,'0')+':'+String(value%60).padStart(2,'0')}
  function communitySoundIcon(muted){return muted?'<svg viewBox="0 0 24 24"><path d="M11 5 6.5 9H3v6h3.5l4.5 4V5Z"/><path d="m16 9 5 6M21 9l-5 6"/></svg>':'<svg viewBox="0 0 24 24"><path d="M11 5 6.5 9H3v6h3.5l4.5 4V5Z"/><path d="M15 9a4 4 0 0 1 0 6M18 6a8 8 0 0 1 0 12"/></svg>'}
  function enhanceCommunityMedia(){document.querySelectorAll('.community-post').forEach(post=>{const media=post.querySelector('.community-media'),video=media?.querySelector('video'),deleteButton=post.querySelector('.community-delete-post'),footer=post.querySelector('footer');if(deleteButton&&footer&&!footer.contains(deleteButton)){deleteButton.innerHTML=communitySvg('trash');deleteButton.setAttribute('aria-label','Удалить публикацию');footer.appendChild(deleteButton)}if(video&&!media.querySelector('.community-video-ui')){video.controls=false;video.muted=true;video.removeAttribute('controls');const ui=document.createElement('div');ui.className='community-video-ui';ui.innerHTML='<button type="button" data-community-sound aria-label="Включить звук">'+communitySoundIcon(true)+'</button><time>00:00</time>';media.appendChild(ui);video.addEventListener('timeupdate',()=>{const timer=ui.querySelector('time');if(timer)timer.textContent=formatCommunityVideoTime(video.duration-video.currentTime)});ui.querySelector('button').addEventListener('click',event=>{event.stopPropagation();video.muted=!video.muted;event.currentTarget.innerHTML=communitySoundIcon(video.muted);event.currentTarget.setAttribute('aria-label',video.muted?'Включить звук':'Выключить звук')})}})}
  function observeCommunityVideos(){if(communityVideoObserver)communityVideoObserver.disconnect();communityVideoObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{const video=entry.target;if(entry.isIntersecting&&entry.intersectionRatio>=.65){document.querySelectorAll('[data-community-video]').forEach(other=>{if(other!==video)other.pause()});video.play().catch(()=>{})}else video.pause()}),{threshold:[0,.65,1]});document.querySelectorAll('[data-community-video]').forEach(video=>communityVideoObserver.observe(video))}
  function toggleCommunityMenu(force){const app=document.querySelector('.community-app'),sidebar=document.querySelector('.community-sidebar'),backdrop=document.querySelector('.community-menu-backdrop');if(!app||!sidebar)return;const open=typeof force==='boolean'?force:!document.body.classList.contains('community-menu-visible');if(open){if(!communityMenuSidebarMarker){communityMenuSidebarMarker=document.createComment('community-sidebar-home');sidebar.parentNode.insertBefore(communityMenuSidebarMarker,sidebar)}if(backdrop&&!communityMenuBackdropMarker){communityMenuBackdropMarker=document.createComment('community-backdrop-home');backdrop.parentNode.insertBefore(communityMenuBackdropMarker,backdrop)}document.body.appendChild(sidebar);if(backdrop)document.body.appendChild(backdrop)}else{if(communityMenuSidebarMarker?.parentNode)communityMenuSidebarMarker.parentNode.insertBefore(sidebar,communityMenuSidebarMarker.nextSibling);if(backdrop&&communityMenuBackdropMarker?.parentNode)communityMenuBackdropMarker.parentNode.insertBefore(backdrop,communityMenuBackdropMarker.nextSibling)}app.classList.toggle('menu-open',open);document.body.classList.toggle('community-menu-visible',open)}
  ...
  function setCommunityChromeVisible(visible){document.body.classList.toggle('community-chrome-hidden',!visible)}
  function bindCommunityScrollChrome(){if(communityScrollBound)return;const scroller=document.querySelector('.scroll');if(!scroller)return;communityScrollBound=true;communityLastScrollTop=scroller.scrollTop;scroller.addEventListener('scroll',()=>{if(!document.querySelector('.view[data-view="community"].active'))return;const current=scroller.scrollTop,delta=current-communityLastScrollTop,atEnd=current+scroller.clientHeight>=scroller.scrollHeight-28;if(atEnd||current<20||delta<-7)setCommunityChromeVisible(true);else if(delta>7)setCommunityChromeVisible(false);communityLastScrollTop=current},{passive:true})}
  function ensureCommunityFixedNav(){const nav=document.querySelector('.community-mobile-nav');if(nav&&nav.parentNode!==document.body){if(!communityNavMarker){communityNavMarker=document.createComment('community-mobile-nav-home');nav.parentNode.insertBefore(communityNavMarker,nav)}document.body.appendChild(nav)}bindCommunityScrollChrome();setCommunityChromeVisible(true)}
  async function loadCommunityFeed(force){ensureCommunityFixedNav();const allowed=new Set(['image','video']),fresh=communityItems.length&&Date.now()-communityFeedLoadedAt<45000;if(!communityItems.length){const cached=readCommunityFeedCache();if(cached){communityItems=[communityDemoPost()].concat(cached.items.filter(item=>allowed.has(normalizeGalleryType(item.type))));communityFeedLoadedAt=Number(cached.savedAt||0);renderCommunityFeed()}}else renderCommunityFeed();if(!force&&fresh)return;if(communityFeedPromise)return communityFeedPromise;const host=document.getElementById('communityFeed');if(host&&!communityItems.length)host.innerHTML='<div class="community-state">Загрузка публикаций…</div>';communityFeedPromise=(async()=>{try{const r=await communityFetch('/api/public/community/feed?telegram_id='+encodeURIComponent(getTelegramId()||0),{cache:'no-store'},9000);if(!r.ok)throw new Error('feed_'+r.status);const j=await r.json(),items=(Array.isArray(j.items)?j.items:[]).filter(item=>allowed.has(normalizeGalleryType(item.type)));communityItems=[communityDemoPost()].concat(items);communityFeedLoadedAt=Date.now();writeCommunityFeedCache(items);renderCommunityFeed();loadCommunityNotificationBadge()}catch(_){if(!communityItems.length)communityItems=[communityDemoPost()];renderCommunityFeed()}finally{communityFeedPromise=null}})();return communityFeedPromise}
  async function deleteCommunityPost(id,button){if(!window.confirm('Удалить публикацию?'))return;if(button)button.disabled=true;try{const r=await fetch('/api/public/community/posts/'+id,{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),initData:S.tg&&S.tg.initData||''})});if(!r.ok)throw new Error();communityItems=communityItems.filter(item=>Number(item.id)!==Number(id));renderCommunityFeed();toast('Публикация удалена')}catch(_){toast('Не удалось удалить публикацию');if(button)button.disabled=false}}
  ...
  function followCommunityAuthor(authorId){return sendCommunityFriendRequest(authorId)}
  function messageCommunityAuthor(authorId){return openCommunityHub('chat',authorId)}
  function communityAvatar(item){return item&&item.avatar?'<img src="'+S.escapeHtml(item.avatar)+'" alt="">':S.escapeHtml(String(item?.name||item?.actor_name||'SY').slice(0,2).toUpperCase())}
  function communityHubModal(){let modal=document.getElementById('communityHubModal');if(!modal){modal=document.createElement('div');modal.id='communityHubModal';modal.className='community-hub';document.body.appendChild(modal)}return modal}
  async function loadCommunityNotificationBadge(){try{const r=await communityFetch('/api/public/community/notifications?telegram_id='+encodeURIComponent(getTelegramId()||0)+'&limit=20',{},7000),j=await r.json(),badge=document.getElementById('communityNotificationBadge');if(badge){badge.hidden=!j.unread;badge.textContent=j.unread>9?'9+':String(j.unread||'')}}catch(_){}}
  async function openCommunityHub(tab='messages',peerId=null){toggleCommunityMenu(false);clearInterval(communityChatPollTimer);communityChatPollTimer=null;setCommunityChromeVisible(true);const modal=communityHubModal();modal.classList.add('show');modal.innerHTML='<section><header><b>SYLVEX Community</b><button type="button" onclick="SYLVEX.closeCommunityHub()">'+communitySvg('close')+'</button></header><main><div class="community-state">Загрузка…</div></main></section>';try{if(tab==='notifications'){const r=await fetch('/api/public/community/notifications?telegram_id='+encodeURIComponent(getTelegramId()||0));if(!r.ok)throw new Error('notifications');const j=await r.json();renderCommunityNotifications(modal,j.items||[]);fetch('/api/public/community/notifications/read',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId()})}).then(loadCommunityNotificationBadge);return}const hr=await fetch('/api/public/community/hub?telegram_id='+encodeURIComponent(getTelegramId()||0));if(!hr.ok)throw new Error('hub');const hub=await hr.json();if(tab==='friends'){renderCommunityFriends(modal,hub);return}if(tab==='chat'){await renderCommunityChat(modal,Number(peerId||0),hub);return}renderCommunityConversations(modal,hub)}catch(_){modal.querySelector('main').innerHTML='<div class="community-state">Не удалось загрузить раздел.<button type="button" onclick="SYLVEX.openCommunityHub(\''+S.escapeHtml(tab)+'\','+Number(peerId||0)+')">Повторить</button></div>'}}
  function closeCommunityHub(){clearInterval(communityChatPollTimer);communityChatPollTimer=null;document.getElementById('communityHubModal')?.classList.remove('show')}
  function renderCommunityConversations(modal,hub){modal.querySelector('header b').textContent='Сообщения';const rows=[{id:0,name:'Общий чат',last_message:'Сообщения всего сообщества'}].concat(hub.conversations||[]).filter((v,i,a)=>a.findIndex(x=>Number(x.id)===Number(v.id))===i);modal.querySelector('main').innerHTML='<nav class="community-hub-tabs"><button onclick="SYLVEX.openCommunityHub(\'messages\')" class="active">Сообщения</button><button onclick="SYLVEX.openCommunityHub(\'friends\')">Друзья</button><button onclick="SYLVEX.openCommunityHub(\'notifications\')">Уведомления</button></nav><div class="community-hub-list">'+rows.map(item=>'<button type="button" onclick="SYLVEX.openCommunityHub(\'chat\','+Number(item.id)+')"><span class="community-avatar">'+communityAvatar(item)+'</span><span><b>'+S.escapeHtml(item.name||'Пользователь')+'</b><small>'+S.escapeHtml(item.last_message||'Открыть диалог')+'</small></span><i>›</i></button>').join('')+'</div>'}
  function renderCommunityFriends(modal,hub){modal.querySelector('header b').textContent='Друзья';const requests=hub.requests||[],friends=hub.friends||[];modal.querySelector('main').innerHTML='<nav class="community-hub-tabs"><button onclick="SYLVEX.openCommunityHub(\'messages\')">Сообщения</button><button class="active">Друзья</button><button onclick="SYLVEX.openCommunityHub(\'notifications\')">Уведомления</button></nav>'+(requests.length?'<h4>Приглашения</h4><div class="community-hub-list">'+requests.map(item=>'<div><span class="community-avatar">'+communityAvatar(item)+'</span><span><b>'+S.escapeHtml(item.name)+'</b><small>Хочет добавить вас в друзья</small></span>'+(item.incoming?'<button onclick="SYLVEX.communityFriendAction('+Number(item.id)+',\'accept\')">Принять</button><button onclick="SYLVEX.communityFriendAction('+Number(item.id)+',\'decline\')">Отклонить</button>':'<i>Отправлено</i>')+'</div>').join('')+'</div>':'')+'<h4>Список друзей</h4><div class="community-hub-list">'+(friends.map(item=>'<button type="button" onclick="SYLVEX.openCommunityHub(\'chat\','+Number(item.id)+')"><span class="community-avatar">'+communityAvatar(item)+'</span><span><b>'+S.escapeHtml(item.name)+'</b><small>Написать сообщение</small></span><i>›</i></button>').join('')||'<p>Список друзей пока пуст.</p>')+'</div>'}
  function communityMessageTime(value){const date=value?new Date(value):new Date();return date.toLocaleDateString([], {day:'2-digit',month:'2-digit'})+' · '+date.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}
  function communityChatMessageHtml(item){const ownUser=S.user||{},avatar=item.author_avatar||(item.is_own?(ownUser.custom_avatar_url||ownUser.photo_url||''):''),name=item.author_name||(item.is_own?(ownUser.display_name||ownUser.first_name):'')||'SYLVEX User';return '<article data-message-id="'+S.escapeHtml(String(item.id||''))+'" class="'+(item.is_own?'own ':'')+(item.pending?'pending':'')+'"><span class="community-message-avatar">'+(avatar?'<img src="'+S.escapeHtml(avatar)+'" alt="">':S.escapeHtml(name.slice(0,2).toUpperCase()))+'</span><div><b>'+S.escapeHtml(name)+'</b><p>'+S.escapeHtml(item.body)+'</p><time>'+(item.is_own?'Отправлено':'Получено')+' · '+S.escapeHtml(communityMessageTime(item.created_at))+'</time></div></article>'}
  async function refreshCommunityChatMessages(peerId,keepPosition=false){const modal=document.getElementById('communityHubModal'),list=modal?.querySelector('.community-chat-messages');if(!list||Number(modal.dataset.peerId)!==Number(peerId))return;try{const r=await fetch('/api/public/community/messages/'+peerId+'?telegram_id='+encodeURIComponent(getTelegramId()||0));if(!r.ok)return;const j=await r.json(),items=j.items||[],signature=items.map(item=>item.id).join(',');if(list.dataset.signature===signature)return;const nearBottom=list.scrollHeight-list.scrollTop-list.clientHeight<80;list.dataset.signature=signature;list.innerHTML=items.map(communityChatMessageHtml).join('')||'<div class="community-state">Начните общение первым.</div>';if(!keepPosition||nearBottom)list.scrollTop=list.scrollHeight}catch(_){}}
  async function renderCommunityChat(modal,peerId,hub){const peer=peerId===0?{name:'Общий чат'}:[...(hub.friends||[]),...(hub.conversations||[])].find(item=>Number(item.id)===peerId)||communityItems.find(item=>Number(item.author_id)===peerId)||{name:'Личные сообщения'};modal.querySelector('header b').textContent=peer.name||peer.author_name||'Чат';modal.dataset.peerId=peerId;modal.querySelector('main').innerHTML='<div class="community-chat-messages"><div class="community-state">Загрузка…</div></div><form class="community-chat-form" onsubmit="SYLVEX.sendCommunityMessage(event,'+peerId+')"><input name="message" maxlength="2000" autocomplete="off" placeholder="Сообщение…" required><button type="submit">'+communitySvg('send')+'</button></form>';await refreshCommunityChatMessages(peerId);clearInterval(communityChatPollTimer);communityChatPollTimer=window.setInterval(()=>refreshCommunityChatMessages(peerId,true),3000)}
  async function sendCommunityMessage(event,recipientId){event.preventDefault();const form=event.currentTarget,input=form.elements.message,body=input.value.trim(),list=form.parentElement.querySelector('.community-chat-messages');if(!body)return;const tempId='pending-'+Date.now(),own=S.user||{};input.value='';list.querySelector('.community-state')?.remove();list.insertAdjacentHTML('beforeend',communityChatMessageHtml({id:tempId,is_own:true,pending:true,body,created_at:new Date().toISOString(),author_name:own.display_name||own.first_name||'Вы',author_avatar:own.custom_avatar_url||own.photo_url||''}));list.scrollTop=list.scrollHeight;try{const r=await communityFetch('/api/public/community/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),recipient_id:recipientId,body,initData:S.tg&&S.tg.initData||''})},9000);if(!r.ok)throw new Error();refreshCommunityChatMessages(recipientId)}catch(_){list.querySelector('[data-message-id="'+tempId+'"]')?.classList.add('failed');toast('Не удалось отправить сообщение')}}
  async function sendCommunityFriendRequest(targetId){if(!targetId||Number(targetId)===Number(getTelegramId()))return;try{const r=await fetch('/api/public/community/friends/request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),target_id:targetId,initData:S.tg&&S.tg.initData||''})});if(!r.ok)throw new Error();toast('Приглашение отправлено')}catch(_){toast('Не удалось отправить приглашение')}}
  async function communityFriendAction(otherId,action){try{const r=await fetch('/api/public/community/friends/'+otherId,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),action,initData:S.tg&&S.tg.initData||''})});if(!r.ok)throw new Error();openCommunityHub('friends')}catch(_){toast('Не удалось изменить список друзей')}}
  function openCommunityNotification(kind,actorId,postId){if(kind==='message')return openCommunityHub('chat',actorId);if(kind==='friend_request'||kind==='friend_accept')return openCommunityHub('friends');closeCommunityHub();const post=document.querySelector('.community-post[data-post-id="'+Number(postId)+'"]');if(post)post.scrollIntoView({behavior:'smooth',block:'center'});if(kind==='comment'&&postId)setTimeout(()=>openCommunityComments(postId),350)}
  function renderCommunityNotifications(modal,items){modal.querySelector('header b').textContent='Уведомления';modal.querySelector('main').innerHTML='<nav class="community-hub-tabs"><button onclick="SYLVEX.openCommunityHub(\'messages\')">Сообщения</button><button onclick="SYLVEX.openCommunityHub(\'friends\')">Друзья</button><button class="active">Уведомления</button></nav><div class="community-notifications">'+(items.map(item=>'<button type="button" class="'+(item.read?'':'unread')+'" onclick="SYLVEX.openCommunityNotification(\''+S.escapeHtml(item.kind)+'\','+Number(item.actor_id||0)+','+Number(item.post_id||0)+')"><span class="community-avatar">'+(item.actor_avatar?'<img src="'+S.escapeHtml(item.actor_avatar)+'" alt="">':S.escapeHtml(String(item.actor_name||'SY').slice(0,2)))+'</span><div><b>'+S.escapeHtml(item.actor_name||'Пользователь')+'</b><p>'+S.escapeHtml(item.body||({like:'поставил(а) лайк',comment:'оставил(а) комментарий',friend_request:'отправил(а) запрос в друзья',message:'отправил(а) сообщение'}[item.kind]||'Новое событие'))+'</p><time>'+S.escapeHtml(communityCommentTime(item.created_at))+'</time></div></button>').join('')||'<div class="community-state">Новых уведомлений нет.</div>')+'</div>'}
  ...
  async function toggleCommunityLike(id,button){const item=communityItems.find(x=>Number(x.id)===Number(id));if(!item||button?.disabled)return;const previous=!!item.liked;item.liked=!previous;item.likes=Math.max(0,Number(item.likes||0)+(item.liked?1:-1));if(button){button.disabled=true;button.classList.toggle('liked',item.liked);button.querySelector('span').textContent=item.likes}try{const r=await communityFetch('/api/public/community/posts/'+id+'/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),initData:S.tg&&S.tg.initData||''})},7000);const j=await r.json();if(!r.ok||j.error)throw new Error(j.error||'like_failed');if(item.liked!==!!j.liked){item.likes=Math.max(0,item.likes+(j.liked?1:-1));item.liked=!!j.liked}if(button){button.classList.toggle('liked',item.liked);button.querySelector('span').textContent=item.likes}}catch(_){item.liked=previous;item.likes=Math.max(0,item.likes+(previous?1:-1));if(button){button.classList.toggle('liked',item.liked);button.querySelector('span').textContent=item.likes}}finally{if(button)button.disabled=false}}
  function renderCommunityPublisher(modal,loading){const allowed=new Set(['image','video']),items=profileGalleryItems.filter(item=>allowed.has(normalizeGalleryType(item.type))),choices=items.map(item=>{const kind=normalizeGalleryType(item.type);return '<label class="community-publish-choice" data-kind="'+kind+'" hidden><input type="checkbox" value="'+S.escapeHtml(String(item.id))+'" data-kind="'+kind+'" onchange="SYLVEX.limitCommunitySelection(this)"><span>'+profileGalleryIcon(kind)+'</span><b>'+S.escapeHtml(String(item.prompt||item.text||'Генерация').slice(0,70))+'</b></label>'}).join(''),typeButton=(kind,label)=>'<button type="button" data-kind="'+kind+'" onclick="SYLVEX.filterCommunityPublisher(\''+kind+'\')"><span>'+profileGalleryIcon(kind)+'</span><b>'+label+'</b></button>',state=loading&&!choices?'<div class="community-publisher-loading"><i></i><b>Загружаем ваши работы…</b><small>Окно уже открыто — повторно нажимать не нужно</small></div>':(choices||'<p>Сначала создайте фото или видео в Pro Studio.</p>');modal.innerHTML='<section><header><div><small>Новая публикация</small><h3>Выберите медиа</h3></div><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><div class="community-publish-types">'+typeButton('image','Фото')+typeButton('video','Видео')+'</div><div class="community-publish-list">'+state+'</div><div class="community-caption-field" hidden><textarea id="communityCaption" maxlength="360" rows="3" placeholder="Добавьте подпись…"></textarea><small>Необязательно</small></div><button class="community-publish-submit" '+(choices?'':'disabled')+' onclick="SYLVEX.publishCommunitySelection()">Опубликовать</button></section>'}
  function openCommunityPublisher(){let modal=document.getElementById('communityPublishModal');if(!modal){modal=document.createElement('div');modal.id='communityPublishModal';modal.className='community-modal';document.body.appendChild(modal)}const hasCache=hydrateProfileGalleryCache();renderCommunityPublisher(modal,!hasCache);modal.classList.add('show');loadProfileGallery(true).then(()=>{if(modal.classList.contains('show'))renderCommunityPublisher(modal,false)})}
  ...
  async function openCommunityComments(id){let modal=document.getElementById('communityCommentsModal');if(!modal){modal=document.createElement('div');modal.id='communityCommentsModal';modal.className='community-modal';document.body.appendChild(modal)}const cached=communityCommentsCache.get(Number(id));modal.innerHTML='<section><header><h3>Комментарии</h3><button onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">'+communitySvg('close')+'</button></header><div class="community-comments">'+(cached?'':'Загрузка…')+'</div><div class="community-replying" hidden></div><form onsubmit="SYLVEX.sendCommunityComment(event,'+id+')"><input type="hidden" name="parent_comment_id"><input name="comment" maxlength="500" placeholder="Написать комментарий…" required><button type="submit">'+communitySvg('send')+'</button></form></section>';modal.classList.add('show');if(cached)renderCommunityComments(modal,id,cached);if(communityCommentsPromises.has(Number(id)))return communityCommentsPromises.get(Number(id));const request=(async()=>{try{const r=await communityFetch('/api/public/community/posts/'+id+'/comments?telegram_id='+encodeURIComponent(getTelegramId()||0),{},8000);if(!r.ok)throw new Error();const j=await r.json(),items=j.items||[];communityCommentsCache.set(Number(id),items);if(Number(modal.dataset.postId||id)===Number(id))renderCommunityComments(modal,id,items)}catch(_){if(!cached&&Number(modal.dataset.postId||id)===Number(id))modal.querySelector('.community-comments').textContent='Не удалось загрузить комментарии.'}finally{communityCommentsPromises.delete(Number(id))}})();communityCommentsPromises.set(Number(id),request);return request}
  ...
  async function sendCommunityComment(event,id){event.preventDefault();const form=event.currentTarget,input=form.elements.comment,body=input.value.trim(),parent_comment_id=Number(form.elements.parent_comment_id.value||0),modal=document.getElementById('communityCommentsModal'),parent=modal?._communityComments?.find(comment=>Number(comment.id)===parent_comment_id);if(!body||form.dataset.sending==='1')return;form.dataset.sending='1';const own=S.user||{},tempId=-Date.now(),pending={id:tempId,author_id:getTelegramId(),body,created_at:new Date().toISOString(),author_name:own.display_name||own.first_name||'Вы',author_avatar:own.custom_avatar_url||own.photo_url||'',parent_comment_id:parent_comment_id||null,reply_to_name:parent?.author_name||'',likes:0,liked:false,is_own:true,pending:true},item=communityItems.find(x=>Number(x.id)===Number(id));input.value='';cancelCommunityReply();const comments=(modal?._communityComments||communityCommentsCache.get(Number(id))||[]).concat([pending]);communityCommentsCache.set(Number(id),comments);if(modal){renderCommunityComments(modal,id,comments);const list=modal.querySelector('.community-comments');list.scrollTo({top:list.scrollHeight,behavior:'smooth'})}if(item){item.comments=Number(item.comments||0)+1;updateCommunityCommentCount(id,item.comments)}try{const r=await communityFetch('/api/public/community/posts/'+id+'/comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),body,parent_comment_id,initData:S.tg&&S.tg.initData||''})},9000);const j=await r.json();if(!r.ok)throw new Error('comment_failed');pending.id=j.comment_id;pending.pending=false;communityCommentsCache.set(Number(id),comments);if(modal&&Number(modal.dataset.postId)===Number(id))renderCommunityComments(modal,id,comments)}catch(_){const rolledBack=comments.filter(comment=>Number(comment.id)!==tempId);communityCommentsCache.set(Number(id),rolledBack);if(item){item.comments=Math.max(0,Number(item.comments||0)-1);updateCommunityCommentCount(id,item.comments)}if(modal&&Number(modal.dataset.postId)===Number(id))renderCommunityComments(modal,id,rolledBack);toast('Не удалось отправить комментарий')}finally{delete form.dataset.sending}}
  ...
  async function loadProfileGallery(force) {
    ...
    try {
      const response = await fetchWithTimeout('/api/public/prostudio/gallery?telegram_id=' + encodeURIComponent(getTelegramId() || 0) + '&limit=100', { cache:'no-store' }, 12000);
...
async function waitGeneration(jobId, options) {
  ...
  let profileSaveRetryTimer = 0;
  const PROFILE_SAVE_PENDING_KEY = 'sylvex-profile-save-pending';
  ...
  function showProfileSaveStatus(text, state) {
    ...
    status.className = 'profile-save-status show ' + (state || 'saving');
    status.textContent = text;
    window.clearTimeout(status._hideTimer);
    if (state !== 'saving') status._hideTimer = window.setTimeout(() => status.classList.remove('show'), 3200);
  }
  function queueProfileSave(body) {
    try { localStorage.setItem(PROFILE_SAVE_PENDING_KEY, JSON.stringify(body)); } catch {}
  }
  function clearQueuedProfileSave() {
    try { localStorage.removeItem(PROFILE_SAVE_PENDING_KEY); } catch {}
  }
  async function persistProfileInBackground(body, silent) {
    if (profileSaveInFlight) { queueProfileSave(body); return false; }
    profileSaveInFlight = true;
    if (!silent) showProfileSaveStatus('Сохраняем настройки…', 'saving');
    try {
      const r = await fetchWithTimeout('/api/public/telegram/profile', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
      }, 12000);
      const j = await r.json();
      if (!r.ok || j.error) throw new Error(j.error || String(r.status));
      clearQueuedProfileSave();
      ...
      showProfileSaveStatus('Настройки сохранены', 'saved');
      return true;
    } catch (_) {
      queueProfileSave(body);
      showProfileSaveStatus('Настройки применены. Сохранение повторится автоматически', 'queued');
      window.clearTimeout(profileSaveRetryTimer);
      profileSaveRetryTimer = window.setTimeout(retryQueuedProfileSave, 8000);
      return false;
    } finally {
      profileSaveInFlight = false;
    }
  }
  function retryQueuedProfileSave() {
    let body = null;
    try { body = JSON.parse(localStorage.getItem(PROFILE_SAVE_PENDING_KEY) || 'null'); } catch {}
    if (body && navigator.onLine !== false) persistProfileInBackground(body, true);
  }
  window.addEventListener('online', retryQueuedProfileSave);
  window.setTimeout(retryQueuedProfileSave, 1500);
  // =====================================================
  ...
  function saveEditProfile() {
    ...
    persistProfileInBackground(body, false);
  }
  ...
  const AI_ASSISTANT_WELCOME = 'Здравствуйте! Я SYLVEX Ассистент. Напишите, что вы хотите сделать, и я помогу.';
  const aiAssistantMessages = [{role:'ai',text:AI_ASSISTANT_WELCOME,welcome:true}];
  function renderAiAssistant() {
    ...
    chat.innerHTML = aiAssistantMessages.length
      ...
      : '<div class="ai-assistant-message ai">'+S.escapeHtml(AI_ASSISTANT_WELCOME)+'</div>';
  ...
  async function sendAiAssistant(event) {
    ...
    renderAiAssistant();
    try {
      const sylvexPrompt = 'Ты — SYLVEX Ассистент внутри SYLVEX AI. Отвечай пользователю от имени SYLVEX, не называй себя ChatGPT или OpenAI. Будь полезным и понятным. Запрос пользователя: '+prompt;
      const response = await fetch('/api/public/prostudio/generate', {method:'POST',headers:{'Content-Type':'application/json','Cache-Control':'no-cache'},cache:'no-store',body:JSON.stringify({telegram_id:getTelegramId(),prompt:sylvexPrompt,mode:'text',category:'text',model:'gpt-5.6',provider:'openai',text_options:{},history:aiAssistantMessages.filter((message)=>!message.loading&&!message.welcome).slice(-10).map((message)=>({role:message.role==='ai'?'assistant':'user',content:message.text})),attachment:null,conversation_id:'',client_request_id:'assistant_'+Date.now().toString(36),language:uiLang()})});
  ...
  function initAiAssistantFab() {
    ...
    const stopPageGesture=(event)=>{if(dragging){event.preventDefault();event.stopPropagation()}};
    fab.addEventListener('touchmove',stopPageGesture,{passive:false});
    fab.addEventListener('pointerdown',(event)=>{event.preventDefault();event.stopPropagation();dragging=true;moved=false;document.documentElement.classList.add('ai-assistant-dragging');fab.classList.add('is-dragging');fab.setPointerCapture(event.pointerId);S.tg?.disableVerticalSwipes?.()});
    fab.addEventListener('pointermove',(event)=>{if(!dragging)return;event.preventDefault();event.stopPropagation();moved=true;fab.style.left=Math.max(-half+4,Math.min(innerWidth-half-4,event.clientX-half))+'px';fab.style.top=Math.max(0,Math.min(innerHeight-size,event.clientY-half))+'px'});
    const finishDrag=(event)=>{if(!dragging)return;dragging=false;document.documentElement.classList.remove('ai-assistant-dragging');fab.classList.remove('is-dragging');const side=event.clientX<innerWidth/2?'left':'right',y=parseFloat(fab.style.top)||100;place(side,y);localStorage.setItem('sylvex-ai-assistant-position',JSON.stringify({side,y}));S.tg?.enableVerticalSwipes?.();if(moved){suppressClick=true;event.preventDefault();event.stopPropagation()}};
  ...
  function applyAiAssistantVisibility(){const visible=localStorage.getItem('sylvex-ai-assistant-visible')!=='0',fab=document.getElementById('aiAssistantFab'),control=document.getElementById('aiAssistantVisibilitySwitch');if(fab)fab.hidden=!visible;if(control)control.classList.toggle('on',visible)}
  function toggleAiAssistantVisibility(){const visible=localStorage.getItem('sylvex-ai-assistant-visible')!=='0';localStorage.setItem('sylvex-ai-assistant-visible',visible?'0':'1');if(visible)closeAiAssistant();applyAiAssistantVisibility()}

  function switchShopTab(tab){const selected=tab==='tokens'?'tokens':'subscriptions';document.querySelectorAll('[data-shop-tab]').forEach(button=>button.classList.toggle('active',button.dataset.shopTab===selected));document.querySelectorAll('[data-shop-panel]').forEach(panel=>panel.hidden=panel.dataset.shopPanel!==selected)}
  function openSpendingStats(){let modal=document.getElementById('spendingStatsModal');if(!modal){modal=document.createElement('div');modal.id='spendingStatsModal';modal.className='community-modal spending-stats-modal';document.body.appendChild(modal)}const spent=Number((S.user&&S.user.tokens_spent)||0);modal.innerHTML='<section><header><div><small>Статистика расходов</small><h3>Токены SYLVEX</h3></div><button type="button" onclick="this.closest(\'.community-modal\').classList.remove(\'show\')">×</button></header><div class="spending-total"><small>Потрачено за всё время</small><b>'+spent.toLocaleString()+' ⚡️</b></div><div class="spending-chart-empty"><span style="height:22%"></span><span style="height:38%"></span><span style="height:31%"></span><span style="height:54%"></span><span style="height:44%"></span><span style="height:70%"></span><span style="height:62%"></span></div><p class="spending-note">Детализация по дням появится после подключения серверной истории списаний. Текущая сумма взята из реальных данных профиля.</p><button class="community-publish-submit" type="button" onclick="this.closest(\'.community-modal\').classList.remove(\'show\');switchView(\'history\')">Открыть историю генераций</button></section>';modal.classList.add('show')}
  ...
  function defaultStudioGridState() {
    return {
      version:2, projectId:'grid_project_'+Date.now().toString(36), title:'Новая цепочка', panX:90, panY:150, zoom:.9,
      nodes:[
        { id:'grid_task_1', type:'task', subtype:'task', x:80, y:150, title:'Задача', prompt:'', status:'IDLE', locked:false, results:[] },
        { id:'grid_text_1', type:'text', subtype:'image_prompt', x:430, y:150, title:'Промпт изображения', prompt:'', status:'IDLE', locked:false, results:[] },
        { id:'grid_image_1', type:'image', subtype:'image', x:780, y:150, title:'Изображение', prompt:'', status:'WAITING', locked:false, results:[] }
      ],
      edges:[{id:'edge_task_text',from:'grid_task_1',fromPort:'task',to:'grid_text_1',toPort:'task',dataType:'task'},{id:'edge_text_image',from:'grid_text_1',fromPort:'text',to:'grid_image_1',toPort:'text',dataType:'text'}]
    };
  }
  ...
  function loadStudioGridState() {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem(STUDIO_GRID_KEY) || localStorage.getItem('sylvex-prostudio-grid-v1') || 'null');
      if (stored && Array.isArray(stored.nodes) && Array.isArray(stored.edges)) studioGridState = normalizeStudioGridState(stored);
    } catch (_) {}
    if (!studioGridState) studioGridState = normalizeStudioGridState(defaultStudioGridState());
  ...
  function applyStudioGridTransform() {
    ...
    if(canvas){canvas.style.backgroundPosition=`${state.panX||0}px ${state.panY||0}px`;canvas.style.backgroundSize=`${22*(state.zoom||1)}px ${22*(state.zoom||1)}px`}
  ...
  function addStudioGridNode(type,position) {
    const state = loadStudioGridState();
  ...
  function deleteStudioGridNode(id) {
    ...
    studioGridSelectedIds.delete(id);
    saveStudioGridState(); renderStudioGrid();
  }
  ...
  function zoomStudioGrid(delta,clientX,clientY) {
    const state = loadStudioGridState();
    const oldZoom=state.zoom||1,newZoom=Math.max(.2,Math.min(2.5,oldZoom+Number(delta||0))),canvas=document.getElementById('studioGridCanvas'),rect=canvas?.getBoundingClientRect();
    if(rect&&Number.isFinite(clientX)&&Number.isFinite(clientY)){const px=clientX-rect.left,py=clientY-rect.top,worldX=(px-state.panX)/oldZoom,worldY=(py-state.panY)/oldZoom;state.panX=px-worldX*newZoom;state.panY=py-worldY*newZoom}
    state.zoom = newZoom;
