...
function renderGeneratedTelegramButton(url, kind) {
  ...
  async function loadCommunityFeed(force){ensureCommunityFixedNav();const fresh=communityItems.length&&Date.now()-communityFeedLoadedAt<30000;if(communityItems.length){renderCommunityFeed();if(!force&&fresh)return}const host=document.getElementById('communityFeed');if(host&&!communityItems.length)host.innerHTML='<div class="community-state">Загрузка публикаций…</div>';try{const r=await fetch('/api/public/community/feed?telegram_id='+encodeURIComponent(getTelegramId()||0));const j=await r.json(),allowed=new Set(['image','video']);communityItems=[communityDemoPost()].concat((Array.isArray(j.items)?j.items:[]).filter(item=>allowed.has(normalizeGalleryType(item.type))));communityFeedLoadedAt=Date.now();renderCommunityFeed();loadCommunityNotificationBadge()}catch(_){if(!communityItems.length)communityItems=[communityDemoPost()];renderCommunityFeed()}}
  ...
  async function loadCommunityNotificationBadge(){try{const r=await fetch('/api/public/community/notifications?telegram_id='+encodeURIComponent(getTelegramId()||0)+'&limit=20'),j=await r.json(),badge=document.getElementById('communityNotificationBadge');if(badge){badge.hidden=!j.unread;badge.textContent=j.unread>9?'9+':String(j.unread||'')}}catch(_){}}
  ...
  async function sendCommunityComment(event,id){event.preventDefault();const form=event.currentTarget,input=form.elements.comment,body=input.value.trim(),parent_comment_id=Number(form.elements.parent_comment_id.value||0),modal=document.getElementById('communityCommentsModal'),parent=modal?._communityComments?.find(comment=>Number(comment.id)===parent_comment_id);if(!body)return;input.disabled=true;try{const r=await fetch('/api/public/community/posts/'+id+'/comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:getTelegramId(),body,parent_comment_id,initData:S.tg&&S.tg.initData||''})});const j=await r.json();if(!r.ok)throw new Error('comment_failed');input.value='';cancelCommunityReply();const item=communityItems.find(x=>Number(x.id)===Number(id)),own=S.user||{};if(item){item.comments=Number(item.comments||0)+1;updateCommunityCommentCount(id,item.comments)}if(modal){const comments=(modal._communityComments||[]).concat([{id:j.comment_id,author_id:getTelegramId(),body,created_at:new Date().toISOString(),author_name:own.display_name||own.first_name||'Вы',author_avatar:own.custom_avatar_url||own.photo_url||'',parent_comment_id:parent_comment_id||null,reply_to_name:parent?.author_name||'',likes:0,liked:false,is_own:true}]);renderCommunityComments(modal,id,comments);const list=modal.querySelector('.community-comments');list.scrollTo({top:list.scrollHeight,behavior:'smooth'})}}catch(_){toast('Не удалось отправить комментарий')}finally{input.disabled=false}}
